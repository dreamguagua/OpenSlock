...
describe("decideStatusUpdate", () => {
  ...
  it("已 done → terminal", () => {
    expect(decideStatusUpdate({ assignee: me, status: "done" }, me, "todo").kind).toBe("terminal");
  });
  ...
  it("人类也受 done 终态约束(与 agent 一致)", () => {
    expect(decideStatusUpdate({ assignee: other, status: "done" }, human, "todo").kind).toBe("terminal");
  });
});
